...
--  Modif GdM (SVGA02X1..SVGA02X2)
--   1. bugfix: Clear_Screen (Buffer) uses Buffer's sizes
--   2. bugfix: drawing in buffer used screen's line_size instead of its width
--   3. inline assembler Ver_Line & Line (in a Buffer)
--   4. added a Put_Buffer to paste a buffer into another one @ (X,Y)
--   5. temporary bugfix: too big Text_Size (64K) hanged some S3 Trio64V+
--      cards under MS-DOS 7.
--      New size works but is a (generous) estimation of maximum text size

--  Modif GdM IV.1999 (SVGA02X3)
--   6. some parameter modes changed to "in out" for buffer manips
--      (buffer's dimensions are read -in-; its data is set -out-)
--   7. added 1600x1200 mode number 288=16#120#
--   8. added Put_Buffer with transparency value

--  Modif GdM 28-Nov-1999 (SVGA02X4)
--   9. accept cards that do not support state buffer
--      (ATI Mach 64)

--  Modif GdM 15-Dec-1999 (SVGA02X5)
--   10. added Frame_rect
