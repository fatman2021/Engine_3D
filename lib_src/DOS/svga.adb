...
--
--  File:        svga.adb
--  Description: 256 color SVGA driver for VBE 1.1 and higher
--  Rev:         0.2
--  Date:        17-sep-1998
--  Author:      Jerry van Dijk
--  Mail:        jdijk@acm.org

--  Modif GdM: see svga.ads
...
package body SVGA is
   ...
   -- Type definitions --
   ----------------------
   subtype Video_Modes is SVGA_Mode range M640x400 .. M1600x1200;
   type Video_Modes_Array is array (Video_Modes) of Boolean;
   ...
   procedure Initialize (SB : in out Screen_Buffer) is
      Size : Natural := SB.Width * SB.Height;
   begin
   ...
   --------------------------------------------------------------------
   procedure Graphics_Mode (Mode : in SVGA_Mode := Autodetect) is
      ...
      procedure Initialize_Vesa is
         ...
         procedure Load_Video_Modes (Info  : in     VbeInfoBlock;
                                     Modes :    out Video_Modes_Array) is
         ...
         begin
            ...
            while Current_Mode /= 16#FFFF# loop
               ...
               for I in Video_Modes loop
                  if Current_Mode = Video_Modes'Enum_Rep (I) then
                     Modes(I)   := True;
                     Mode_Found := True;
                  end if;
               end loop;
   ...
   begin
      ...
      if Mode = Autodetect then
         for I in reverse Video_Modes loop
      ...
      if Linear_Mode then
         if (Current_Mode.ModeAttributes and Linear_Mode_Present) = 0 or
           Current_Mode.PhysBasePtr = 0 then
            Linear_Mode := False;
         end if;
      end if;
   ...
   begin
      if Y1 = Y2 then
         ...
         ASM(
"   .align 2,0x90  " & LF &
"0: movb %0, (%3) ; addl %2, %3 ; decl %1 ; jns 0b",
             No_Output_Operands,
             (byte'Asm_Input    ("b", Color),           -- %0 = value
              integer'Asm_Input ("r", Bottom-Top),      -- %1 = count
              integer'Asm_Input ("r", Buffer.Width),    -- %2 = step
              address'Asm_Input ("r", Buffer.Data(X + Top * Buffer.Width)'address))); -- %3 = start
      end if;
   end Ver_Line;
   ...
   procedure Line (X1    : in X_Loc;
      ...
      Right  : Integer := Integer (X2);
      Top    : Integer := Integer (Y1);
      Bottom : Integer := Integer (Y2);
   ...
   procedure Line (Buffer :    out Screen_Buffer;
      ...
      Top    : Integer := Integer (Y1);
      Bottom : Integer := Integer (Y2);
      D_X, D_Y, D_X_2, D_Y_2, S_X, S_Y, Line_S_Y : Integer;
--      Limit, Left_Top_Line   : Integer;
   begin
      if X1 = X2 then
         if Y1 = Y2 then
...
"   jns   0b",
             ...
             (byte'Asm_Input    ("b", Color),           -- %0 = Color
              integer'Asm_Input ("r", D_Y),             -- %1 = I in reverse 0 .. D_Y
              integer'Asm_Input ("r", D_X_2 - D_Y),     -- %2 = Limit := D_X_2 - D_Y
              address'Asm_Input ("r", Buffer_Data(Left + Buffer.Width * Top)'address), -- %3 = start
...
"   jns   0b",
             No_Output_Operands,
             (byte'Asm_Input    ("b", Color),           -- %0 = Color
              integer'Asm_Input ("r", D_X),             -- %1 = I in reverse 0 .. D_X
              integer'Asm_Input ("r", D_Y_2 - D_X),     -- %2 = Limit := D_Y_2 - D_X;
              address'Asm_Input ("r", Buffer_Data(Left + Buffer.Width * Top)'address), -- %3 = start
              integer'Asm_Input ("g", S_X),        -- %4
              integer'Asm_Input ("g", Line_S_Y),   -- %5
              integer'Asm_Input ("g", D_X_2),      -- %6
   ...
   procedure Circle (X      : in X_Loc;
      ...
      Correct : Integer := Integer (2 * X_Size / Y_Size);
   begin
      Check_Graphics_Mode;
   ...
   procedure Put_Buffer (X      : in X_Loc;
      ...
      Index  : Natural := 0;
      X2     : X_Loc   := X + X_Loc (Buffer.Width - 1);
      Y2     : Y_Loc   := Y + Y_Loc (Buffer.Height - 1);
   begin
      Check_Graphics_Mode;
   ...
   procedure Put_Buffer (Source      : in     Screen_Buffer;
      ...
      Index  : Natural := 0;
      I_Line : Natural;
      X2     : X_Loc   := X + X_Loc (Source.Width - 1);
      Y2     : Y_Loc   := Y + Y_Loc (Source.Height - 1);
   begin
      Check_Graphics_Mode;
