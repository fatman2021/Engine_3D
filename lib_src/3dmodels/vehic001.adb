...
( 1=>( -10, 360,  10),
 ...
 85=>(   0, 300,  14)
      ...
      for i in 1..vehicle_001.num_of_points loop
        vehicle_001.baseobj(i).X:= integer(float(vehicle_001.baseobj(i).X)
           * (fl_screen_virtual_size / 430.0));
        vehicle_001.baseobj(i).Y:= integer(float(vehicle_001.baseobj(i).Y - 160)
           * (fl_screen_virtual_size / 430.0));
        vehicle_001.baseobj(i).Z:= integer(float(vehicle_001.baseobj(i).Z)
           * (fl_screen_virtual_size / 540.0));
      end loop;

      vehicle_001.num_of_faces:= polys'length(1);
      for i in polys'range(1) loop
        case i is
         when 2|4|6|19|21|23|44|46|48|70|72|74 =>
                        cmin:= 144; cmax:= 159; -- jaune
         when 1|3|5|18|20|22|45|47|49|69|71|73 =>
                        cmin:=  64; cmax:=  79; -- bleu f.
         when 34 =>     cmin:= 131; cmax:= 137; -- rouge
         when 41..43|75|76|89 =>
                        cmin:=  16; cmax:=  31; -- vitre cockpit
         when 30|32 =>  cmin:= 217; cmax:= 239; -- react.
         when 83..86=>  cmin:=  96; cmax:= 106; -- museau
         when others => cmin:= 160; cmax:= 191; -- caisse
        end case;
        vehicle_001.faces(i):=
         ((polys(i,1),polys(i,4),polys(i,3),polys(i,2)), -- anticlockwise
          false,  null, 1,1, int_min,int_max, cmin, cmax);
      end loop;

      vehicle_001.center:= (0.0,0.0,fl_screen_virtual_size);
      vehicle_001.id:= "vehicle_001     ";
      Init_object(vehicle_001.all);
